...
ga_path=$(find $ga_dir/ | grep $region | grep "masked_dense.gz$\|masked_dense$" | head -n 1)

#echo $ga_path

if [ ! -f $ga_path ]; then
    echo -e "genotype array file not found in $ga_dir. Searched for region $region, suffix masked_dense"
    exit
fi
