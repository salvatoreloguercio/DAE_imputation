...
def parse_arguments(user_arguments):
    ...
    parser.add_argument('inlist',
                        metavar='table of genotype_array input and output filenames',
                        type=lambda x: is_valid_path(parser, x, valid_extensions=['tsv']),
                        help="Table of genotype array files in VCF format, files to be imputed")
...
def main(argv):
    ...
    with open(args.inlist, 'r') as file:
        for line in file:
            ...
            # Split the line into two columns (assuming they are separated by a tab '\t')
            input_f, output_f = line.strip().split('\t')
            
            try:    
                new_df, known_indexes = process_data(args.posfile, input_f)
            ...
            start = timeit.default_timer()
            export_vcf(args.posfile, out, input_f, output_f)
